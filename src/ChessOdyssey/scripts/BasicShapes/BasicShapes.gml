function ColPoint(position, owner = undefined) constructor{
	self.position = position;				//Vec3
	self.owner = owner;						//Instance ID
...
function ColSphere(position, radius, owner = undefined) constructor{
	self.position = position;				//Vec3
	...
	static NearestPoint = function(vec3){
		...
		var scaled_dist = dist.Dot(self.radius);
		return scaled_dist.Add(self.position);
	};
}

function ColAABB(position, half_extents, owner = undefined) constructor {
	self.position = position;				//Vec3
	...
	static CheckAABB = function(aabb){
		...
		return ((box_min.x <= other_max.x) && (box_max.x >= other_min.x) && (box_min.y <= other_max.y) && (box_max.y >= other_min.y) && 
				(box_min.z <= other_max.z) && (box_max.z >= other_min.z));
	};
	...
	static CheckRay = function(ray, hit_info){
		...
		var tmin = max(min(t1, t2), min(t3, t4), min(t5, t6));
		var tmax = min(max(t1, t2), max(t3, t4), max(t5, t6));
	...
	static CheckLine = function(line){
		var ray = NewColRayFromPoints(line.start, line.finish);
...
function ColPlane(normal, distance, owner = undefined) constructor{
...
function ColRay(origin, direction, owner = undefined) constructor{
...
function ColLine(start, finish, owner = undefined) constructor{
